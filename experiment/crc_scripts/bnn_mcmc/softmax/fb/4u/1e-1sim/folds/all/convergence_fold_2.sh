...
python3 $BASE_PATH/experiment/research/bnn/proto_bnn_mcmc.py \
    ...
    --output_dir "$SRC_PATH/converging/hmc_4u_1e-1sim_3nlfs_0lag_1e6res/"$STEP_SIZE"ss/2e7burn/" \
    --bnn_weights_file "$SRC_PATH/converging/hmc_4u_1e-1sim_3nlfs_0lag_1e6res/"$STEP_SIZE"ss/2e7burn/last_weights.json" \
    --num_hidden 4 \
    --cpu 1 \
    --cpu_cores 4 \
    --gpu 0 \
